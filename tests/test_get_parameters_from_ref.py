...
def test_get_parameters_from_ref():
    ...
    params = get_request_body_parameters_from_ref(full_spec, ref="#/components/schemas/CreateBehaviorGroupRequest")
    assert len(params) == 1
    assert params[0].name == "display_name"
    assert params[0].type == "string"

    params = get_request_body_parameters_from_ref(full_spec, ref="#/components/schemas/UpdateBehaviorGroupRequest")
    assert len(params) == 4
    assert params[0].name == "display_name"
    assert params[0].type == "string"
    ...
    assert params[1].aggregate_info == {"type": "string", "format": "uuid",
